describe("Criação de Filmes com Sucesso", function () {
  ...
  it("Deve ser possível criar filme com ano de lançamento igual ao ano atual", function () {
    cy.request({
      method: "POST",
      url: "api/movies",
      auth: {
      ...
      .then(function (resposta) {
        ...
        expect(resposta.body.description).to.equal("O navio afunda.");
        expect(resposta.body.durationInMinutes).to.equal(185);
        expect(resposta.body.releaseYear).to.equal(2024);
      })
      .then(function (resposta) {
        movieId = resposta.body.id;
      });
...
describe("Criação de Filmes com Falha", function () {
  ...
  it("Não é possível criar um filme sendo usuario comum", function () {
    let dadosUser;
    let token;
  ...
  describe("Filmes com falha e hooks de before e after", function () {
    ...
    it("Não deve ser possível criar filme sem ano de lançamento", function () {
      cy.request({
        method: "POST",
        url: "api/movies",
        auth: {
